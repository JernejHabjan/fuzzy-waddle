...
- [x] Implement Stages 0–14 sequentially on the integration branch.
- [ ] Complete Stage 15 runtime matrix, baseline calibration, validation, repair and closure.
...
The integration PR remains draft/not release-validated through Stages 0–14; tests and fixtures are authored there while executable checks remain deferred. Stage 15 runs the complete integrated harness, establishes baseline variance, reviews predeclared numerical thresholds and reports primary metrics, guardrails, fixture coverage and candidate/baseline results. Intermediate green gates are not passing releases. A single win, hand-picked seed, or subjective “looked smarter” playthrough is not sufficient. No automatic merge.

## Implementation roadmap

The authoritative execution entry point is [00-start-here.md](759-skirmish-ai/00-start-here.md). Implement Stages 0–14 sequentially on one isolated integration branch, author tests and fixtures without running them, then perform builds, playtests, whole-solution review and full validation in Stage 15. The latest boundary policy is one stage per user request: complete its manual requirement/evidence audits, commit, push/verify and stop. Do not wait for intermediate PR merges or automatically enter the next stage. Runtime command validation is always active.

The stage acceptance lists are retained and strengthened in the packets below, with source paths, contracts, algorithms, defaults, debug obligations and fixture instructions. Release Gates A–D are capability groupings; all executable evidence is collected at Stage 15.
...
Implement only the selected/resumed stage on one isolated integration branch. Through Stage 14,
author but do not run tests, lint, type checks, builds or smokes; manually audit all acceptance.
Run and repair the complete verification ladder only at Stage 15. Apply H1–H9,
