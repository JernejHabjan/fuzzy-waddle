...
- [x] Receive and record reviewer approval for D1-D6.
- [ ] Implement the approved prefab, trigger, tests, and map placement using the existing bird assets.
- [ ] Complete automated verification and the manual playtest matrix.

## Classification and scope

- **Lane:** this began as a `decision-pr`; D1-D6 are now approved, so the plan is implementation-ready once #575 supplies the typed shark dependency. This PR does not change runtime behavior.
...
- Approved: depend on #575 first. It must add a runtime shark prefab registered under a typed `ObjectNames.Shark` contract; #628 then identifies threats through `ActorIndexSystem` and exact actor type. Do not infer sharks from texture keys, constructor names, arbitrary tags, or substring matching.

### D2 — Seagull artwork

- Approved: reuse the current Little Muncher bird artwork already present in Probable Waffle's `animals` atlas, including the existing idle, idle-flap, takeoff, and flight animation keys. A distinct seagull art pass is deferred and is not required for #628.

### D3 — Ambient versus synchronized state

- Approved: keep the seagull client-local and non-authoritative. Use wall-clock polling/tweens for presentation, read the synchronized shark positions, and never include seagull state in lockstep, snapshots, saves, or reconnect restoration.

### D4 — Trigger tuning

- Approved: hardcode named implementation constants of 192 world pixels for the reaction radius and 250 ms for the idle polling cadence; do not expose a Phaser Editor field. Trigger once when any active `ObjectNames.Shark` is within Euclidean world-space distance, ignoring destroyed or inactive actors.

### D5 — Takeoff path and lifetime

- Approved: make takeoff one-shot. Play the takeoff animation, transition to the looping flight animation, fly horizontally away from the triggering shark with a consistent upward screen-space lift, and destroy the seagull after it exits a padded camera/world bound or reaches a short safety timeout. Do not return to the water during the current scene session.

### D6 — Water placement and sound

- Approved: treat water placement as a Phaser Editor/map-author responsibility and keep the prefab non-colliding. Add no per-seagull sound in #628; maps may reuse the existing attributed `seagulls` ambient `SoundEffectMarker` where desired.
...
### Stage 2 — Scene-backed water seagull prefab

- [ ] Reuse the current Little Muncher bird frames from the existing `animals` atlas and its `bird-idle`, `bird-idle-flap`, `bird-fly-off`, and `bird-fly` animation keys without changing the asset pipeline.
...
- [ ] Resolve `ActorIndexSystem` through the typed scene-service helper and poll only while the seagull is idle, using named hardcoded constants for the approved 192-pixel radius and 250-ms cadence.
...
- A distinct seagull visual treatment is deferred; #628 intentionally reuses the current Little Muncher bird artwork.
...
```text
Continue issue #628 from docs/ai/628-seagull-water-prefab.md. D1-D6 are approved and authoritative: depend on #575's typed indexed shark first, reuse the current Little Muncher bird artwork and animations, keep the seagull client-local and non-authoritative, hardcode the documented 192-pixel reaction radius and 250-ms polling cadence, use the approved one-shot fly-away lifecycle, and leave water placement and ambient sound to map authors. Implement the stages in order, keep WaterSeagull.scene and WaterSeagull.ts synchronized, preserve TreeBird behavior, run the listed checks with NX_DAEMON=false, complete the omission and final closure audits, and update the draft PR with visual/manual-playtest evidence. Do not merge.
```
