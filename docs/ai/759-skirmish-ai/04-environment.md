...
Read [runbook](00-start-here.md), [shared decisions](01-shared-decisions.md), [debug contract](06-debug-panel.md), and the research sections for bases/fortification/component coverage. Dependencies: implemented 0–9. Author fixtures now; execute final acceptance in Stage 15.
...
1. Track per-order objective progress from logical distance/phase change and application outcomes. Default no-progress window 200 ticks; known stun/freeze pauses the movement deadline within its observed remaining duration, and slow adjusts expected travel. Queued work waiting behind acknowledged predecessors is not a failed order.
...
7. Global inactivity watchdog runs through proposals: if eligible idle worker/producer or inactive army persists >200 ticks, publish corrective action or explicit blocker/deadline. After four failed equivalent candidates expose a terminal reason and choose another goal, not indefinite retry.
