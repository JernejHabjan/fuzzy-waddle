...
Read after [the runbook](00-start-here.md). These defaults close design choices for all stages. Numbers are initial authored tuning values, not measured optimal balance; Stage 15 evaluates and tunes them with recorded evidence. Preserve actual game costs, damage, capacity, and faction rules from definitions.
