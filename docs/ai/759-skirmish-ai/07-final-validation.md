...
Model: `gpt-5.6-sol`, effort `xhigh`. [Runbook](00-start-here.md) policy: this is the final required stage; all deferred execution happens here. Stage 16 search/learning is excluded. Prior stages must be `implemented_unvalidated`, not falsely marked passing. Start with the complete progress/verification ledger and the candidate commit.
...
## 15C — pure and real simulation matrix

Run every retained Level A/Level B fixture in the research document, plus each packet's added cases. Give each a stable ID, version, fixed seed, setup, perturbation tick, maximum simulation duration, observed conditions and exact assertions. Run each focused deterministic fixture three times; compare commands, authoritative hash, AI digest and first-differing normalized path. Test permutations of candidate input order, render rates, pause/speed, and save continuation.
...
| Recovery/modes | Proxy/wall exploit resolved or terminal explained fallback; blocked/depleted/lost assets recover; neutral claim ties canonical; own-unit allied support; timed/stockpile/last-building objectives and concession lead to exactly one result/score |
...
- Normal completes legal opening plus purposeful attack or expansion within 15 simulated minutes on viable stable scenarios; each critical disruption either recovers within its authored deadline or produces an explicit valid terminal/fallback reason.
- No eligible worker/producer/army remains unexplained idle beyond the 200-tick watchdog window. A valid supply, threat, affordability, capacity, disabled, mode or unreachable reason is recorded with a next check/recovery deadline.
...
- Median worker idle time, supply-block ticks and producer idle time improve by at least 10% over a comparable nonzero baseline, or retain zero when already zero. If scenario design makes this inappropriate, document the measured cause, revise the metric with rationale, and rerun the full relevant matrix; do not waive a behavioral failure by changing thresholds.
...
- Inspect applicable repo skills in `plugins/fuzzy-waddle-skills/skills/`. Update repo workflow/task-tracking guidance only for reusable lessons; add AI-specific guidance to the appropriate Phaser or narrowly scoped AI skill if the final architecture warrants it. Read the skill-creator instructions before creating/updating a skill. Do not turn this task's defer-until-final policy into a global rule. Do not edit unrelated user-installed/system skills.
