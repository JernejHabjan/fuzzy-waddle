...
- [x] Separate presentation state from authoritative mission state and record the product decisions.
- [x] Receive and record approval for D1-D5, including the prohibition on generative-AI assets.
...
This began as a `decision-pr` because the issue contains a Warcraft-style reference image but no written acceptance criteria. D1-D5 are now approved, so the plan is implementation-ready. Implementation remains a separate step and has not started in this planning PR.
...
- Use Phaser-authored frames, shapes, typography, and existing licensed project assets. The issue screenshot is composition reference only; do not copy its artwork, borders, icons, or textures. Do not generate or include generative-AI artwork, icons, textures, illustrations, audio, or other assets.

## Confirmed decisions

### D1 — Layout scope

- Approved: implement the full-viewport two-list/detail composition above, with main quests on the left and optional quests on the right; stack the lists above details at narrow viewports.

### D2 — Objective category mapping

- Approved: show `primary` and `secondary` under `Main quests`; show `optional` and revealed `hidden` kinds under `Optional quests`; and show `tutorial` in a compact `Guidance` subsection. Show `failure` objectives only after reveal, in the main column with a failure treatment. Keep mission-message history in the existing dialogue/history surface rather than mixing it into the quest list.

### D3 — Undiscovered objectives

- Approved: show one inert `Undiscovered quest` placeholder for each objective whose runtime status is `hidden`, in the section selected from its authored kind, exposing only the count and category. Never expose its id, title, description, checklist, focus target, history, or status reason. Replace the placeholder in place when the objective is revealed.

### D4 — Modal and input behavior

- Approved: keep simulation running in every game mode, but treat the open quest log as a local blocking overlay. Consume pointer/wheel input over the overlay, suppress world selection/orders/cursors while open, and close with `Done`, `Escape`, or a click on the dimmed backdrop. Reuse the existing `Mission log` entry point; defer a global hotkey until there is a central remappable input contract.
...
- Approved: ship Phaser scene/code-authored frames and status/category glyphs using existing licensed fonts and UI assets. Do not add per-objective art metadata in this issue; reserve an icon slot so a separately licensed, human-created asset pass can add authored icons later without restructuring rows.
- Constraint: do not create or include generative-AI artwork, icons, textures, illustrations, audio, or other assets.
...
- The desktop layout matches the approved reference composition without copying third-party artwork or using generative-AI assets.
...
- The screenshot is not an asset license. Any new bitmap/audio asset must be human-created or sourced under verified reuse rights with attribution; generative-AI assets are prohibited.
