...
- Skirmish AI runtime entry points are `pnpm ai:skirmish:opening`, `pnpm ai:skirmish:production` and `pnpm ai:skirmish:land-sequences`. Summarize the newest artifact with `pnpm ai:skirmish:report -- --failures-only --details`, or use `--report <path> --scenario <ID>` for a named artifact; do not dump full JSON before narrowing. See the AI runtime E2E guide for batching details; optional IDE launch profiles are not test authority.
