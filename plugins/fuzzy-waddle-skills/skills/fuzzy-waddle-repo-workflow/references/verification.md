...
- Skirmish AI runtime entry points are `pnpm ai:skirmish:opening`, `pnpm ai:skirmish:production` and `pnpm ai:skirmish:land-sequences`. Summarize the emitted artifact with `pnpm ai:skirmish:report -- --failures-only --details`; do not dump the full JSON before narrowing to a scenario. See the #759 Stage 15 operator guide for IDE and batching details.
