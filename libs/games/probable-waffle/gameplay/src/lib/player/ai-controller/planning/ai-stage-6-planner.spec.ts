...
describe("planAiStage6V1", () => {
  ...
  it("services every lane deterministically even when no manager can dispatch work", () => {
    const state = createAiBrainStateV1({ playerNumber: 1, faction: FactionType.Tivara, profile, tick: 0, archetypeId: "opening:balanced" });
    const result = planAiStage6V1(createStage2Observation(), state, [], [], profile);
    expect(result.state.lanes.map((lane) => lane.lane)).toEqual([
      "essential_economy", "supply_production", "scouting", "army_threat", "optional_infrastructure_tech"
    ]);
  ...
  it("expires provisional work once and preserves applied spending", () => {
    ...
    const withReservations = {
      ...state,
      reservations: [
        { claimId: "claim:expired", subjectKey: "actor:worker-1", ownerPlanId: "plan:opening", state: { kind: "provisional" as const, expiresAt: { clock: "simulation" as const, unit: "tick" as const, dueTick: 10, persistence: "save" as const } }, prerequisites: [], createdTick: 0 },
        { claimId: "claim:spent", subjectKey: "resource:wood", ownerPlanId: "plan:opening", state: { kind: "applied_spending" as const, appliedTick: 1 }, prerequisites: [], createdTick: 0 }
      ]
    };
