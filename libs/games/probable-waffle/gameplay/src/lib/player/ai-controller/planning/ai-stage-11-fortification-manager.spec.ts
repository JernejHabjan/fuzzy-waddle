...
const catalog: AiCapabilityCatalogV1 = {
  ...
  entries: [
    {
      capabilityId: "worker", family: "build", sourceObjectName: ObjectNames.TivaraWorker, effectiveLevel: 1,
      movementDomains: ["ground"], targetDomains: [], produces: [], constructs: [ObjectNames.Wall, ObjectNames.WatchTower, ObjectNames.Stairs, ObjectNames.Olival], researches: [], gathers: [],
      housingCapacity: null, housingCost: 1, cargoCapacity: null
    },
    {
      capabilityId: "wall", family: "passive", sourceObjectName: ObjectNames.Wall, effectiveLevel: 1,
      movementDomains: ["ground"], targetDomains: [], produces: [], constructs: [], researches: [], gathers: [],
      housingCapacity: null, housingCost: null, cargoCapacity: null,
      constructionProfile: { resourceCost: { [ResourceType.Stone]: 25 }, footprintRadiusTiles: 0, visionRange: 10, navigableHeight: 42, enterHeight: 64, exitHeight: 64 }
    },
    {
      capabilityId: "tower", family: "attack", sourceObjectName: ObjectNames.WatchTower, effectiveLevel: 1,
      movementDomains: ["ground"], targetDomains: ["ground", "water"], produces: [], constructs: [], researches: [], gathers: [],
      housingCapacity: null, housingCost: null, cargoCapacity: null,
      constructionProfile: { resourceCost: { [ResourceType.Stone]: 250 }, footprintRadiusTiles: 0, visionRange: 18, navigableHeight: 104, enterHeight: 64, exitHeight: 64 }
    },
    {
      capabilityId: "stairs", family: "passive", sourceObjectName: ObjectNames.Stairs, effectiveLevel: 1,
      movementDomains: ["ground"], targetDomains: [], produces: [], constructs: [], researches: [], gathers: [],
      housingCapacity: null, housingCost: null, cargoCapacity: null,
      constructionProfile: { resourceCost: { [ResourceType.Stone]: 25 }, footprintRadiusTiles: 0, visionRange: 10, navigableHeight: 24, enterHeight: 0, exitHeight: 64 }
    },
    {
      capabilityId: "gate-double", family: "passive", sourceObjectName: ObjectNames.Olival, effectiveLevel: 1,
      movementDomains: ["ground"], targetDomains: [], produces: [], constructs: [], researches: [], gathers: [],
      housingCapacity: null, housingCost: null, cargoCapacity: null,
      constructionProfile: { resourceCost: { [ResourceType.Stone]: 50 }, footprintRadiusTiles: 0, visionRange: 8, navigableHeight: 0, enterHeight: 0, exitHeight: 0 }
    }
  ]
};

function actor(actorId: string, objectName: ObjectNames, x: number, y: number, relation: "self" | "enemy" = "self"): AiObservedActorV1 {
  return {
    ...
    ...(relation === "self" ? { mainBuilding: { status: "known" as const, value: objectName === ObjectNames.Sandhold, observedTick: 100 } } : {})
  };
}
...
function observation(extraActors: readonly AiObservedActorV1[] = [], tick = 100): AiObservationV1 {
  ...
  for (let y = 0; y <= 20; y += 1) {
    for (let x = 0; x <= 20; x += 1) {
      ...
      cells.push({ tileKey: `${x},${y}`, position: { x, y, z: 0 }, groundPassable: !anchor, waterPassable: false, elevation: 0, observedBlocked: anchor });
    }
  }
  return {
    ...
    generation: 1,
    tick,
    actors: [actor("main", ObjectNames.Sandhold, 5, 10), actor("worker", ObjectNames.TivaraWorker, 6, 10), actor("worker-reserve", ObjectNames.TivaraWorker, 6, 11), actor("enemy", ObjectNames.SkaduweeWorker, 18, 10, "enemy"), ...extraActors],
    resources: [{ resourceType: ResourceType.Stone, stockpile: 5_000, reservedUnspent: 0, obligationsDue: 0, deliveredIncomePerMinute: { status: "known", value: 20, observedTick: tick } }],
    threatSummary: { observedTick: tick, visibleEnemyActorIds: ["enemy"], rememberedEnemyActorIds: [], observedCapabilityFamilies: ["attack"] },
    map: {
...
function threatenedState(): AiBrainStateV1 {
  ...
  return {
    ...initial,
    bases: [{ baseId: "base:main:main", anchorActorId: "main", memberActorIds: ["main", "worker", "worker-reserve"], active: true, lifecycle: "active", anchorPosition: { x: 5, y: 10, z: 0 } }],
    skirmish: {
      ...initial.skirmish,
      incidents: [{ incidentId: "incident:front", baseId: "base:main:main", regionId: "access:main", hostileActorIds: ["enemy"], kind: "army_pressure", confidencePermille: 1000, severity: 700, createdTick: 80, expiresAt: aiDeadline(300) }]
    }
  };
}
...
describe("AiStage11FortificationManagerV1", () => {
  ...
  it("H-27 rejects an unanchored open-field wall that has no measured choke value", () => {
    ...
    const proposal = manager.propose({
      ...open,
      map: { ...open.map!, constructionCells: open.map!.constructionCells!.map((cell) => ({ ...cell, groundPassable: true, observedBlocked: false })) }
    }, threatenedState());
  ...
  it("WALL-03 records marginal tower coverage and only definition-supported target domains", () => {
    ...
    expect(graph?.nodes.filter((node) => node.kind === "stair").every((stair) => stair.defenderPostReachable)).toBe(true);
  });
  ...
  it("keeps a partial construction requested and does not unlock its connected successor", () => {
    ...
    expect(proposal.intents.some((intent) => intent.kind === "construct" && intent.siteKey.includes(successor.nodeId))).toBe(false);
  });
  ...
  it("H-28 reuses stable node effects and WALL-04 turns a missing finished node into one bounded repair", () => {
    ...
    const firstNode = first.statePatch!.fortifications![0]!.graph!.nodes.find((node) => node.effectId === firstConstruct?.effectId);
    const reservedState: AiBrainStateV1 = {
      ...plannedState,
      reservations: firstNode && firstConstruct ? [{
    ...
    const breachedState = {
      ...threatenedState(),
      squads: [{ squadId: "squad:defense" as const, role: "defense" as const, domain: "ground" as const, actorIds: ["guard" as ActorId], objectiveId: null, state: "ready" as const }],
      fortifications: [{
        ...plan,
        lifecycle: "active" as const,
        graph: { ...plan.graph!, nodes: plan.graph!.nodes.map((node) => node.nodeId === root.nodeId ? { ...node, lifecycle: "finished" as const, completedActorId: "destroyed-wall" as ActorId, retryAfterTick: 0 } : node) }
      }]
    };
    const breached = manager.propose(observation([actor("guard", ObjectNames.TivaraMacemanMale, 7, 10)], 200), breachedState);
    ...
    expect(breached.intents.some((intent) => intent.kind === "move" && intent.reasonCode.includes("breach_defenders"))).toBe(true);
  });
  ...
  it("WALL-04 repairs a destroyed stair without recreating its retained connected component", () => {
        ...
        graph: {
          ...
          nodes: plan.graph.nodes.map((node) =>
            node.nodeId === stair.nodeId
              ? { ...node, lifecycle: "finished" as const, completedActorId: "old-stair" as ActorId, retryAfterTick: 0 }
              : node.nodeId === dependency.nodeId
                ? { ...node, lifecycle: "finished" as const, completedActorId: "retained-segment" as ActorId, retryAfterTick: 400 }
                : node
          )
        }
      }]
    ...
    expect(proposal.intents.some((intent) => intent.kind === "construct" && intent.objectName === ObjectNames.Stairs)).toBe(true);
  });
});
