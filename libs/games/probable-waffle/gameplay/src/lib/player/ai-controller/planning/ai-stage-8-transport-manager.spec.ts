...
const built = buildAiAccessGraphV1({
  generation: 1, staticRevision: 1, dynamicRevision: 1, threatRevision: 0, builtTick: 20,
  continuationCursor: 0, includeAirRegion: true,
  cells: [
    { x: 0, y: 0, ground: true, water: false, elevation: 0, groundNeighborKeys: [], knowledge: "known_static", clearance: 2 },
    { x: 1, y: 0, ground: false, water: true, elevation: 0, groundNeighborKeys: [], knowledge: "known_static", clearance: 2 },
    { x: 2, y: 0, ground: false, water: true, elevation: 0, groundNeighborKeys: [], knowledge: "known_static", clearance: 2 },
    { x: 3, y: 0, ground: false, water: true, elevation: 0, groundNeighborKeys: [], knowledge: "known_static", clearance: 2 },
...
const routeRequest = {
  ...
  toNodeId,
  capabilities: {
    moverDomains: ["ground" as const], targetDomains: [], waterTransportSeats: 4, airTransportSeats: 0,
    requiredPassengerSeats: 2, requiredClearance: 1
  },
  firingNodeIds: []
};
...
const catalog: AiCapabilityCatalogV1 = {
  ...
  unsupported: [],
  entries: [{
    capabilityId: "CommonBoat:level:1", family: "transport", sourceObjectName: ObjectNames.CommonBoat,
    effectiveLevel: 1, movementDomains: ["water"], targetDomains: [], produces: [], constructs: [], researches: [], gathers: [],
    housingCapacity: null, housingCost: 1, cargoCapacity: 4
  }]
};
...
describe("AiStage8TransportManagerV1 CommonBoat lifecycle", () => {
  it("reserves, gathers, boards, travels, unloads, regroups and releases mission ownership", () => {
    ...
    let state = createAiBrainStateV1({ playerNumber: 1, faction: FactionType.Tivara, profile, tick: 0, archetypeId: "balanced" });
    const plan = createAiTransportPlanV1({
      planId: "transport:island-1", routeRequest, route, tick: 20, missionKind: "island_establishment", estimatedTravelTicks: 40,
      passengers: [
    ...
    const boat = {
      ...
      capabilities: [{ id: "boat", family: "transport", level: 1, domains: ["water" as const], targetDomains: [], capacity: { status: "known" as const, value: 4, observedTick: 20 } }],
      containerState: { status: "known" as const, value: { capacity: 4, passengerIds: [], pendingPassengerIds: [], mobileDomains: ["water" as const] }, observedTick: 20 }
    };
    const atPickup = [actor("worker-1", ObjectNames.TivaraWorker, fromNodeId, 0), actor("guard-1", ObjectNames.TivaraMacemanMale, fromNodeId, 0), boat];
    ...
    expect(manager.propose(observation(atPickup, 24), state).intents).toContainEqual(expect.objectContaining({ kind: "board", transportId: "boat-1" }));

    const loaded = atPickup.map((entry) =>
      entry.actorId === "boat-1"
        ? { ...entry, containerState: { status: "known" as const, value: { capacity: 4, passengerIds: ["guard-1", "worker-1"], pendingPassengerIds: [], mobileDomains: ["water" as const] }, observedTick: 25 } }
    ...
    const landed = loaded.map((entry) => ({ ...entry, logicalPosition: { status: "known" as const, value: { x: 3, y: 0, z: 0 }, observedTick: 26 } }));
    for (const tick of [26, 27]) {
      result = manager.propose(observation(landed, tick), state);
      state = { ...state, transport: result.statePatch!.transport! };
    }
    expect(state.transport[0]?.phase).toBe("unloading");
    expect(manager.propose(observation(landed, 28), state).intents).toContainEqual(expect.objectContaining({ kind: "unload", transportId: "boat-1" }));

    const unloaded = landed.map((entry) =>
      entry.actorId === "boat-1"
        ? { ...entry, containerState: { status: "known" as const, value: { capacity: 4, passengerIds: [], pendingPassengerIds: [], mobileDomains: ["water" as const] }, observedTick: 29 } }
        : { ...entry, containedInActorId: null, accessNodeId: { status: "known" as const, value: toNodeId, observedTick: 29 } }
    );
    for (const tick of [29, 30, 31]) {
      result = manager.propose(observation(unloaded, tick), state);
      state = { ...state, transport: result.statePatch!.transport! };
    }
    expect(state.transport[0]).toEqual(expect.objectContaining({ phase: "completed", passengerIds: [], transportIds: [] }));
  });
  ...
  it("bounds a lost indispensable passenger instead of producing endless boats", () => {
    ...
    const plan = createAiTransportPlanV1({
      planId: "transport:loss", routeRequest, route, tick: 20, missionKind: "army_transfer", estimatedTravelTicks: 40,
      passengers: [{ actorId: "worker-dead", role: "combat", indispensable: true, handoff: "squad" }]
    });
    const manager = new AiStage8TransportManagerV1(() => catalog);
    const result = manager.propose(observation([], 21), { ...initial, transport: [{ ...plan, phase: "reserving", lifecycle: { ...plan.lifecycle!, recoveryAttempt: 3 } }] });
  ...
  it("cancels a second plan that tries to own the same passenger and destination", () => {
    const profile = createAiProfileConfigV1(ProbableWaffleAiDifficulty.Medium);
    const initial = createAiBrainStateV1({ playerNumber: 1, faction: FactionType.Tivara, profile, tick: 0, archetypeId: "balanced" });
    const passengers = [{ actorId: "worker-1", role: "combat" as const, indispensable: true, handoff: "squad" as const }];
    const first = createAiTransportPlanV1({
      planId: "transport:a", routeRequest, route, tick: 20, missionKind: "army_transfer", estimatedTravelTicks: 40, passengers
    });
    const second = createAiTransportPlanV1({
      planId: "transport:b", routeRequest, route, tick: 20, missionKind: "army_transfer", estimatedTravelTicks: 40, passengers
    });
  ...
  it("revalidates an unsafe landing and enters bounded recovery without unloading", () => {
    ...
    const source = createAiTransportPlanV1({
      planId: "transport:unsafe", routeRequest, route, tick: 20, missionKind: "army_transfer", estimatedTravelTicks: 40,
    ...
    const result = new AiStage8TransportManagerV1(() => catalog).propose(
      observation([
        ...
        {
          ...actor("boat-1", ObjectNames.CommonBoat, built.waterNodeByTileKey.get("1,0")!, 1),
          containerState: { status: "known" as const, value: { capacity: 4, passengerIds: ["guard-1"], pendingPassengerIds: [], mobileDomains: ["water" as const] }, observedTick: 20 }
        },
        ...enemies
      ], 21),
  ...
  it("hands off surviving cargo already ashore after its transport is lost", () => {
    ...
    const initial = createAiBrainStateV1({ playerNumber: 1, faction: FactionType.Tivara, profile, tick: 0, archetypeId: "balanced" });
    const source = createAiTransportPlanV1({
      planId: "transport:survivor", routeRequest, route, tick: 20, missionKind: "army_transfer", estimatedTravelTicks: 40,
      passengers: [{ actorId: "guard-1", role: "combat", indispensable: true, handoff: "squad" }]
    });
    const plan = {
      ...source,
  ...
  it("stops an in-flight operation when its route generation is invalidated", () => {
    ...
    const initial = createAiBrainStateV1({ playerNumber: 1, faction: FactionType.Tivara, profile, tick: 0, archetypeId: "balanced" });
    const source = createAiTransportPlanV1({
      planId: "transport:stale-route", routeRequest, route, tick: 20, missionKind: "army_transfer", estimatedTravelTicks: 40,
      passengers: [{ actorId: "guard-1", role: "combat", indispensable: true, handoff: "squad" }]
    });
    const plan = {
      ...source,
      ...
      transportIds: ["boat-1"],
      lifecycle: {
        ...source.lifecycle!, assignedTransportIds: ["boat-1"],
        seatAssignments: [{ transportId: "boat-1", passengerIds: ["guard-1"] }], assignedCapacity: 1,
        pickupTransferId: route.pickupCandidates[0]!.transferId,
        landingTransferId: route.landingCandidates[0]!.transferId
      }
    };
    const staleObservation = observation([
      ...
      {
        ...actor("boat-1", ObjectNames.CommonBoat, built.waterNodeByTileKey.get("1,0")!, 1),
        containerState: { status: "known" as const, value: { capacity: 4, passengerIds: ["guard-1"], pendingPassengerIds: [], mobileDomains: ["water" as const] }, observedTick: 21 }
      }
    ], 21);
    const result = new AiStage8TransportManagerV1(() => catalog).propose(
    ...
    expect(result.statePatch?.transport[0]).toEqual(
      expect.objectContaining({ phase: "recovering", lifecycle: expect.objectContaining({ terminalReason: "route_generation_invalidated" }) })
    );
    expect(result.intents).toHaveLength(0);
  });
  ...
  it.each(["boarding", "transit", "unloading"] as const)("preserves canonical save identity during %s", (phase) => {
    ...
    const initial = createAiBrainStateV1({ playerNumber: 1, faction: FactionType.Tivara, profile, tick: 0, archetypeId: "balanced" });
    const plan = createAiTransportPlanV1({
      planId: `transport:save:${phase}`, routeRequest, route, tick: 20, missionKind: "army_transfer", estimatedTravelTicks: 40,
      passengers: [{ actorId: "guard-1", role: "combat", indispensable: true, handoff: "squad" }]
    });
    const operation = {
      ...plan,
    ...
    const boat = {
      ...actor("boat-1", ObjectNames.CommonBoat, built.waterNodeByTileKey.get(atLanding ? "3,0" : "1,0")!, atLanding ? 3 : 1),
      containerState: {
