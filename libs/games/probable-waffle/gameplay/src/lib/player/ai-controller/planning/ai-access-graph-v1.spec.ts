...
const cells: readonly AiAccessCellV1[] = [
  ...
  { x: 2, y: 0, ground: true, water: false, elevation: 0, groundNeighborKeys: [], knowledge: "known_static", clearance: 2 }
];
...
describe("Stage 8 access graph", () => {
  ...
  it("keeps unknown regions pending and service failures technical", () => {
    ...
    const request = {
      ...
      toNodeId,
      capabilities: {
        moverDomains: ["ground" as const], targetDomains: [], waterTransportSeats: 2,
        airTransportSeats: 0, requiredPassengerSeats: 1, requiredClearance: 1
      },
      firingNodeIds: []
    };
    expect(queryAiAccessRouteV1(built.graph, request)).toEqual(expect.objectContaining({ kind: "pending", reason: "unknown_region" }));
  ...
  it("rejects a connected region that cannot clear the requested footprint", () => {
    ...
    expect(queryAiAccessRouteV1(built.graph, {
      queryId: "query:formation-clearance",
      kind: "movement",
      fromNodeId: nodeId,
      toNodeId: nodeId,
      capabilities: {
        moverDomains: ["ground"], targetDomains: [], waterTransportSeats: 0,
        airTransportSeats: 0, requiredPassengerSeats: 1, requiredClearance: 3
      },
      firingNodeIds: []
  ...
  it("rejects a wide formation at a narrow connected corridor but admits a single-tile mover", () => {
    const corridor = build([
      ...
      { x: 1, y: 0, ground: true, water: false, elevation: 0, groundNeighborKeys: ["0,0", "2,0"], knowledge: "known_static", clearance: 1 },
      { x: 2, y: 0, ground: true, water: false, elevation: 0, groundNeighborKeys: ["1,0"], knowledge: "known_static", clearance: 2 }
    ]);
    ...
    const request = {
      ...
      toNodeId,
      capabilities: {
        moverDomains: ["ground" as const], targetDomains: [], waterTransportSeats: 0,
        airTransportSeats: 0, requiredPassengerSeats: 1, requiredClearance: 2
      },
      firingNodeIds: []
    };
    ...
    expect(queryAiAccessRouteV1(corridor.graph, {
      ...request,
      capabilities: { ...request.capabilities, requiredClearance: 1 }
    })).toEqual(expect.objectContaining({ kind: "direct", routeNodeIds: expect.arrayContaining([fromNodeId, toNodeId]) }));
  });
  ...
  it("returns a transport route when legal carrier production can satisfy missing capacity", () => {
    ...
    expect(queryAiAccessRouteV1(built.graph, {
      ...
      toNodeId,
      capabilities: {
        moverDomains: ["ground"], targetDomains: [], waterTransportSeats: 0, airTransportSeats: 0,
        canProduceWaterTransport: true, requiredPassengerSeats: 2, requiredClearance: 1
      },
      firingNodeIds: []
