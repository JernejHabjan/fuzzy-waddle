...
- **09** Added `StateHashService`: djb2 hash of all actors (`id:health:lx:ly:owner`) every 60 ticks; broadcasts via new `state-hash` TwoWayCommunicator (MP only); each client compares incoming peer hashes against its local stored hash for that tick; mismatch → `console.error([DESYNC] tick/player/local/remote)` + Phaser text overlay at depth 9999. Added `ProbableWaffleStateHashEvent` interface; server relays without processing.
