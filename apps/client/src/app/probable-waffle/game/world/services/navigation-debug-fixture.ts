...
import Wall, { type WallPrefabKey } from "../../prefabs/buildings/tivara/wall/Wall";
import Stairs, { type StairsPrefabKey } from "../../prefabs/buildings/tivara/stairs/Stairs";
...
export class NavigationDebugFixture {
  ...
  private createConnectedElevatedRoute(): void {
    this.createStairs({ x: 640, y: 640, z: 0 }, "topLeft");
    this.createWall({ x: 672, y: 624, z: 0 }, "empty");
    this.createWall({ x: 704, y: 608, z: 0 }, "empty");
    this.createActor(ObjectNames.WatchTower, { x: 736, y: 592, z: 0 });
    this.createStairs({ x: 608, y: 656, z: 0 }, "bottomRight");
  }

  private createDisconnectedWall(): void {
    this.createWall({ x: 832, y: 640, z: 0 }, "empty");
    this.createWall({ x: 864, y: 624, z: 0 }, "bottomLeftBottomRight");
  }

  private createNarrowCorridor(): void {
    this.createWall({ x: 640, y: 800, z: 0 }, "full");
    this.createWall({ x: 704, y: 768, z: 0 }, "full");
  ...
  private createSameTileHeightReference(): void {
    this.createWall({ x: 928, y: 704, z: 0 }, "empty");
    this.createActor(ObjectNames.TivaraWorkerMale, { x: 928, y: 704, z: 0 }, 1);
  }

  private createWall(position: Vector3Simple, wallKey: WallPrefabKey): void {
    const actor = this.createActor(ObjectNames.Wall, position);
    if (actor instanceof Wall) {
      actor.updateWall(wallKey);
    }
  }

  private createStairs(position: Vector3Simple, stairsKey: StairsPrefabKey): void {
    const actor = this.createActor(ObjectNames.Stairs, position);
    if (actor instanceof Stairs) {
      actor.updateStairs(stairsKey);
    }
  }
