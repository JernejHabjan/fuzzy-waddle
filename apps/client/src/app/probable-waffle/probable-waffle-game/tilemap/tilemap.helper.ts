...
export class TilemapHelper {
  ...
  static adjustTileWorldWithVerticalOffset( // todo move to IsoHelper
  ...
  static get tileCenterOffset(): number { // todo move to IsoHelper
    return MapSizeInfo.info.tileHeightHalf;
  }

  static getTileWorldCenterByTilemapTileXY( // todo move to IsoHelper
    tileXY: Vector2Simple,
    tileCenterOptions: TileCenterOptions = null
  ): Vector2Simple {
  ...
  createTilemap() {
    ...
    const tilemapLayer = this.mapHelper.tilemapLayer = tilemap.createLayer(tilemap.layers[0].name, tileSetImages) as Phaser.Tilemaps.TilemapLayer;
