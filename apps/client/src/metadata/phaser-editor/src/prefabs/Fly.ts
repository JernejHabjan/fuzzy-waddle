...
export default class Fly extends Phaser.GameObjects.Container {
	...
	constructor(scene: Phaser.Scene, x?: number, y?: number) {
		...
		// leg_back_left
		const leg_back_left = scene.add.image(-15.549315681627625, -2.634144277323962, "fly-squasher-spritesheet", "fly/leg-back");
		leg_back_left.setOrigin(1, 1);
		leg_back_left.flipX = true;
		this.add(leg_back_left);

		// leg_front_right
		const leg_front_right = scene.add.image(12.450684318372382, 31.365855722676038, "fly-squasher-spritesheet", "fly/leg-front");
		leg_front_right.setOrigin(0, 0);
		this.add(leg_front_right);

		// leg_front_left
		const leg_front_left = scene.add.image(-13.549315681627625, 31.365855722676038, "fly-squasher-spritesheet", "fly/leg-front");
		leg_front_left.setOrigin(1, 0);
		leg_front_left.flipX = true;
		this.add(leg_front_left);

		// leg_middle_right
		const leg_middle_right = scene.add.image(12.450684318372382, 8.365855722676038, "fly-squasher-spritesheet", "fly/leg-middle");
		leg_middle_right.setOrigin(0, 0.5);
		this.add(leg_middle_right);

		// leg_middle_left
		const leg_middle_left = scene.add.image(-13.549315681627625, 7.365855722676038, "fly-squasher-spritesheet", "fly/leg-middle");
